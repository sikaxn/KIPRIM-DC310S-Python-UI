...
class DC310SGUI:
    ...
    def setup_core_controls(self, frame):
        ...
        self.on_button = ttk.Button(frame, text="Output ON", command=lambda: self.set_output(1), state='disabled')
        self.on_button.grid(row=1, column=0, pady=5)
        self.off_button = ttk.Button(frame, text="Output OFF", command=lambda: self.set_output(0), state='disabled')
        ...
        self.meas_voltage = tk.Label(frame, text="---", font=("Arial", 24, "bold"), fg="blue")
        self.meas_voltage.grid(row=4, column=1)

        tk.Label(frame, text="Current (A):").grid(row=4, column=2, sticky="w")
        self.meas_current = tk.Label(frame, text="---", font=("Arial", 24, "bold"), fg="green")
        self.meas_current.grid(row=4, column=3)

        tk.Label(frame, text="Power (W):").grid(row=4, column=4, sticky="w")
        self.meas_power = tk.Label(frame, text="---", font=("Arial", 24, "bold"), fg="orange")
    ...
    def get_output_state(self):
        return self.output_voltage > 0.5
    ...
    def set_voltage(self):
        try:
            value = float(self.voltage_entry.get())
