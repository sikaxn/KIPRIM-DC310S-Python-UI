...
class DC310SGUI:
    def __init__(self, root):
        ...
        self.setup_tx_rx_indicators()

        # Start periodic update
        self.root.after(1000, self.periodic_refresh)

    def setup_tx_rx_indicators(self):
        self.indicator_frame = tk.Frame(self.root)
        self.indicator_frame.grid(row=10, column=1, sticky="se", padx=5, pady=5)
        tk.Label(self.indicator_frame, text="TX", font=("Arial", 10)).pack(side="left")
        self.tx_light = tk.Label(self.indicator_frame, text="●", fg="red", font=("Arial", 12))
        self.tx_light.pack(side="left", padx=(2, 10))
        tk.Label(self.indicator_frame, text="RX", font=("Arial", 10)).pack(side="left")
        self.rx_light = tk.Label(self.indicator_frame, text="●", fg="red", font=("Arial", 12))
        self.rx_light.pack(side="left")

    def indicate_tx(self):
        self.tx_light.config(fg="green")
        self.root.update_idletasks()
        self.root.after(300, lambda: self.tx_light.config(fg="red"))

    def indicate_rx(self):
        self.rx_light.config(fg="green")
        self.root.update_idletasks()
        self.root.after(300, lambda: self.rx_light.config(fg="red"))
    ...
    def load_reset_settings(self):
        if not os.path.exists(RESET_SETTINGS_FILE):
            default = {"timer": "reset on output off", "energy": "reset on output off"}
    ...
    def setup_presets_panel(self, frame):
        ...
        tk.Button(frame, text="LOAD", font=("Arial", 14, "bold"), bg="lightblue", command=self.load_selected_preset).pack(pady=10, fill="x")
        tk.Button(frame, text="Save Current Setting", command=self.save_current_as_preset).pack(pady=5, fill="x")
        ...
        for idx, key in enumerate(["timer", "energy"]):
            cb = ttk.Combobox(reset_frame, values=["reset on output on", "reset on output off", "no reset"], state="readonly", width=20)
    ...
    def load_or_create_presets(self):
        if not os.path.exists(PRESET_FILE):
            presets = {"USB Power Supply": {"voltage": 5.0, "current": 3.0}, "Lead Acid Battery": {"voltage": 13.7, "current": 3.0}}
    ...
    def set_voltage(self):
        self.send_command(f"voltage {self.voltage_entry.get()}")
    ...
    def load_selected_preset(self):
        ...
        if not preset:
            return
        if self.output_voltage > 0 and self.preset_click_stage == 0:
    ...
    def refresh_measurements(self):
        ...
        power = self.output_voltage * (float(i_str) if i_str else 0)
        self.output_power = power
        self.meas_power.config(text=f"{power:.2f}" if power else "---")

        if self.output_voltage > 0.5:
            self.elapsed_seconds += 1
            self.energy_ws += self.output_power
            if self.auto_reset_mode["timer"] == "reset on output on":
                self.reset_timer()
            if self.auto_reset_mode["energy"] == "reset on output on":
                self.reset_energy()
        else:
            if self.auto_reset_mode["timer"] == "reset on output off":
                self.reset_timer()
            if self.auto_reset_mode["energy"] == "reset on output off":
                self.reset_energy()
        ...
        bg = "light green" if self.output_voltage > 0.5 else "light gray"
